			...
			initialize:function(x,y,a,b,photo){
				...
				this.on("enterframe",function(){
					...
					if(this.intersect(enemywall)){
						...
						else if(friendScore == 2){var hata = new Hata(130,70);}
						else{alert("a");}
		...
		var Icon =Class.create(Sprite,{
	 		initialize:function(x,y,a,b,photo,team){
				...
				this.on("touchstart",function(){
					...
					if(friendCount<5) {						//最大MOB数が５＞以下
					for(var i=0;i<=5;i++){					//flagが空いてるのを探すためのfor
					if(friendCharaBox[this.team][i]==undefined){		//iの値が何も入ってないとき真
						friendCharaNumber=i;						//i=Number
						friendCharaBox[this.team][i]=new Bear(this.x,this.y);//flagの空いてる場所にクマ召喚
						friendCount++;
					...
					if(enemyCharaBox[this.team][i]==undefined){
						enemyCharaNumber=i;
						enemyCharaBox[this.team][i]=new Enemy(this.x,this.y+32);
						enemyCount++;
						break;
					}}}}
				});
...
//アイコン精製味方---------------------------------------------------
		var icon1 =new Icon(10,130,50,40,"chara1.png",0);//(x,y,ｱｲｺﾝｻｲｽﾞx,ｱｲｺﾝｻｲｽﾞy,image)
